...
import (
	...
	"github.com/frozenpine/ctp4go/mduser"
)
...
func TestMdApi(t *testing.T) {
	...
	md, err := mduser.NewMduserApi(
		t.Context(),
		mduser.WithLibPath(libPath),
		mduser.WithFrontAddr(front),
		mduser.WithBrokerID("9999"),
		mduser.WithUserID("164889"),
		// mduser.WithUserPassEnvKey("RDQH_CTP_USER_PASS"),
		mduser.WithUserPass("022010blue@safe"),
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = md.Initialize(
		mduser.WithMduserState(
			mduser.WithStateResponsor(mduser.Connected, md.Login),
			mduser.WithStateResponsor(mduser.LoginSuccess, func() error {
